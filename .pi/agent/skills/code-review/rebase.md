...
A rebase onto the fixed point is mid-flight with conflicts. Do NOT resolve in the main session. Spawn a `worker` sub-agent via the `subagent` tool with a task carrying the repo path and these rules:
