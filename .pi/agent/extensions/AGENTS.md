...
- **`bg-run.ts`** — runs a long/noisy shell command in the background with no agent involved, for deterministic commands that need tracking (logs, status, kill) but no reasoning.
