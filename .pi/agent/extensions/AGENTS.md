...
- **`lsp-feedback.ts`** (+ sibling `lsp-feedback.lua`) — after every edit, route file through nvim for format + safe code-actions + diagnostics. Errors and warnings trigger an LLM auto-fix loop (told to fix root cause, not suppress with ignore/disable directives); any issues left unfixed fire a `notify` so they're surfaced for manual review. Per-edit lint runs a fast-linter allowlist only: `SLOW_LINTERS` (e.g. semgrep — network rule fetch, multi-second) are skipped here since async results land after the file budget returns; SAST belongs at pre-commit/CI cadence, not every edit. Slow linters run on save in interactive nvim; `plugin/lint.lua` gates them off entirely under the agent's `vim.g.pi_agent` so the feedback driver's own writes don't trigger them. On `session_start` the headless nvim is warm-spawned + feedback lua preloaded via a deferred `setTimeout(0)` (off pi's startup critical path) so the first edit skips spawn + init.lua + LSP-attach latency. The spawn sets `vim.g.pi_agent` via `--cmd` (pre-config); cosmetic plugins (`plugin/colorscheme|completion|debug|mini|neo-tree|snacks|ui|misc|treesitter.lua`) early-return on that flag, so the agent nvim loads only LSP/format/lint — ~40% faster start, identical servers/rules. Interactive nvim unaffected (flag unset).
