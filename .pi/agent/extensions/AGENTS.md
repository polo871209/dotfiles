...
- **`subagent.ts`** — `/subagent` offloads long-running tasks to an isolated `pi` agent (single-layer, no recursion), running visibly in its own tmux pane instead of a hidden background process — watch, scroll, or step in directly. Agents defined in `~/.pi/agent/agents/*.md` — each file's body is the subagent's entire system prompt, no shared preamble or other context added, for an unambiguous clean start; `hidden: true` agents stay invocable only when named explicitly (e.g. from a skill). Use it to offload research/recon/implementation with role-appropriate model defaults. tmux-only: the tool isn't available outside a tmux session.
