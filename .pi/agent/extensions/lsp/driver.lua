...
local function open_buf_nowait(file)
    ...
    if existing and vim.api.nvim_buf_is_valid(existing) and mtimes[file] == current_mtime then
        touch(existing)
        return existing
    end
...
function M.status()
    ...
    for _, c in ipairs(vim.lsp.get_clients()) do
        table.insert(servers, { name = c.name, id = c.id, root = c.root_dir, bufs = vim.tbl_count(c.attached_buffers or {}) })
    end
